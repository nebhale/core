...
| `charge_current_request_max` | `Charge Current Request (Max)` | `current` |  |  | `A` |  |  |  |  |
| `charge_current_request` | `Charge Current Request` | `current` |  |  | `A` |  |  |  |  |
| `charge_energy_added` | `Energy Added` | `energy` |  |  | `kWh` | `units.RoundingValueTemplate` |  |  |  |
...
| `charger_actual_current` | `Charger Current` | `current` |  |  | `A` |  |  |  |  |
| `charger_phases` | `Charger Phases` |  |  | `mdi:sine-wave` |  |  |  |  |  |
| `charger_power` | `Charger Power` | `power` |  |  | `kW` |  |  |  |  |
| `charger_voltage` | `Charger Voltage` | `voltage` |  |  | `V` |  |  |  |  |
...
| `elevation` | `Elevation` |  |  | `mdi:image-filter-hdr` | `unitsCfg.Distance.DistanceShortUnits()` | `unitsCfg.Distance.DistanceShortValueTemplate()` |  |  |  |
...
| `inside_temp` | `Inside Temp` | `temperature` | `measurement` |  | `°C` | `units.RoundingValueTemplate` |  |  |  |
...
| `outside_temp` | `Outside Temp` | `temperature` | `measurement` |  | `°C` | `units.RoundingValueTemplate` |  |  |  |
...
| `speed` | `Speed` |  |  | `mdi:speedometer` | `unitsCfg.Distance.SpeedUnits()` | `unitsCfg.Distance.SpeedValueTemplate()` |  |  |  |
...
| `time_to_full_charge` | `Time to Charged` | `duration` |  | `mdi:timer` | `h` |  |  |  |  |
